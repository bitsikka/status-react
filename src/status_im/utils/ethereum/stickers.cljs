...
(defn pack-data [web3 contract pack-id cb]
  ...
  (ethereum/call web3
                 ...
                 (fn [_ data]
                   (cb (when-not (empty? (subs data 2)) (abi-spec/decode (subs data 2) ["bytes4[]" "address" "bool" "uint256" "uint256" "bytes"]))))))
...
(defn owned-tokens [web3 contract address cb]
  ...
  (ethereum/call web3
                 ...
                 (fn [_ data]
                   (cb (when-not (empty? (subs data 2)) (first (abi-spec/decode (subs data 2) ["uint256[]"])))))))
